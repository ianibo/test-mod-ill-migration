...
public class WmsHostLmsService extends BaseHostLmsService {
  ...
  public CirculationClient getCirculationClient(
      Institution institution,
      ISettings settings, String address
  ) {
    String wms_api_key = settings.getSettingValue(
        institution,
        SettingsData.SETTING_WMS_API_KEY
    );
    String wms_api_secret = settings.getSettingValue(
        institution,
        SettingsData.SETTING_WMS_API_SECRET
    );
    String wms_lookup_patron_endpoint = settings.getSettingValue(
        institution,
        SettingsData.SETTING_WMS_LOOKUP_PATRON_ENDPOINT
    );
  ...
  List getLookupStrategies() {
    [
      [
        ...
        precondition: { PatronRequest pr -> return ( pr.oclcNumber != null ) },
        strategy: { PatronRequest pr, WmsHostLmsService service, ISettings settings, IHoldingLogDetails holdingLogDetails ->
      ...
      [
        ...
        precondition: { PatronRequest pr -> return ( pr.isbn != null && pr.title != null ) },
        strategy: { PatronRequest pr, WmsHostLmsService service, ISettings settings, IHoldingLogDetails holdingLogDetails ->
            return service.lookupViaConnector(
                pr.institution,
  ...
  List<ItemLocation> lookupViaConnector(
      Institution institution,
      String query,
      ISettings settings
  ) {
    ...
    String wms_connector_address = settings.getSettingValue(
        institution,
        SettingsData.SETTING_WMS_CONNECTOR_ADDRESS
    );
    String wms_connector_username = settings.getSettingValue(
        institution,
        SettingsData.SETTING_WMS_CONNECTOR_USERNAME
    );
    String wms_connector_password = settings.getSettingValue(
        institution,
        SettingsData.SETTING_WMS_CONNECTOR_PASSWORD
    );
    String wms_api_key = settings.getSettingValue(
        institution,
        SettingsData.SETTING_WMS_API_KEY
    ...
    String wms_registry_id = settings.getSettingValue(
        institution,
