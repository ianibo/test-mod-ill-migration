package com.k_int.ill.reporting;
...
public class ReportService {
     ...
     */
    public FileFetchResult generateReport(
        ...
        if (reportId || fallbackReportResource) {
            ...
            try {
                ...
                InputStream logoInputStream = getImage(institution, imageId);

                // Execute the jasper report
                result.inputStream = jasperReportService.executeReport(
                    institution,
                    report?.fileDefinition,
                    schema,
                    identifiers,
                    logoInputStream,
                    fallbackReportResource
                );
        ...
        if (name && description && domain && contentType && filename && (multipartFile != null)) {
            Report.withTransaction { tstatus ->
                ...
                if ((report == null) && (id != null)) {
                    ...
                    FileDefinitionCreateResult fileCreationResult = fileService.create(
                        institution,
                        FileType.REPORT_DEFINITION,
                        description,
                        multipartFile
                    );
            ...
            result = new ReportCreateUpdateResult();
     ...
     */
    public String getPullSlipReportId(Institution institution) {
        return(institutionSettingsService.getSettingValue(institution, SettingsData.SETTING_PULL_SLIP_REPORT_ID));
    }
     ...
     */
    public int getMaxRequestsInPullSlip(Institution institution) {
        return(institutionSettingsService.getSettingAsInt(
            institution,
            SettingsData.SETTING_PULL_SLIP_MAX_ITEMS,
            100,
            false
        ));
    }
     ...
     */
    public int getMaxRequestsInPullSlipManual(Institution institution) {
        return(institutionSettingsService.getSettingAsInt(
            institution,
            SettingsData.SETTING_PULL_SLIP_MAX_ITEMS_MANUAL,
            100,
            false
        ));
    }
     ...
     */
    public String getPullSlipLogoId(Institution institution) {
        // Get hold of the id for the logo
        return(institutionSettingsService.getSettingValue(institution, SettingsData.SETTING_PULL_SLIP_LOGO_ID));
    }
     ...
     */
    public InputStream getImage(Institution institution, String imageId) {
        // The inputStream for the logs
        InputStream imageInputStream = null;

        // Do we have an id for the logo
        if (imageId != null) {
            imageInputStream = fileService.fetch(Institution, imageId).inputStream;
        }
