...
public class StaticRouterService extends BaseRouterService {
	InstitutionSettingsService institutionSettingsService;

	public StaticRouterService() {
		super('StaticRouterService', 'Static Routing Service');
	}
	...
	public List<RankedSupplier> findMoreSuppliers(
		...
		log.debug("StaticRouterService::findMoreSuppliers(${patronRequest.id})");
		List<RankedSupplier> result = []

		Institution institution = patronRequest.institution;
		String static_routing_str = institutionSettingsService.getSettingValue(institution, SettingsData.SETTING_STATIC_ROUTES);
