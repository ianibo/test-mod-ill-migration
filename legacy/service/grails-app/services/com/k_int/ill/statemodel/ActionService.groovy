...
public class ActionService {
    ...
    /**
     * Retrieves a request for the supplied id and institution
     * @param patronRequestId The id of the request to be returned
     * @param institution The institution that the caller belongs to
     * @return The PatronRequest if it is found for the institution otherwise null if it is not found
     */
    public PatronRequest getRequest(String patronRequestId, Institution institution) {
        return(PatronRequest.findByIdAndInstitution(patronRequestId, institution));
    }

    /**
     * Checks a patron request to see if it belongs to the supplied institution
     * @param patronRequestId The id of the request to be checked
     * @param institution The institution that we want to check it belongs to
     * @return true if it belongs to the institution otherwise false
     */
    public boolean isRequestForInstitution(String patronRequestId, Institution institution) {
        return(getRequest(patronRequestId, institution) != null);
    }

    /**
     * Checks a patron request to see if it belongs to the supplied institution
     * @param patronRequest The request to be checked
     * @param institution The institution that we want to check it belongs to
     * @return true if it belongs to the institution otherwise false
     */
    public boolean isRequestForInstitution(PatronRequest patronRequest, Institution institution) {
        return(patronRequest.institution.getId().equals(institution.getId()));
    }
     ...
     */
    public Map executeAction(String patronRequestId, String action, Object parameters, boolean userGenerated = false ) {
        Map result = [ : ];
        ...
        if (patronRequestId) {
            try {
                ...
                if (patronRequest == null) {
                    ...
                    result =  executeAction(patronRequest, action, parameters, userGenerated);
                }
            } catch (Exception e) {
     ...
     * @return A map containing the result of the processing
     */
    public Map executeAction(PatronRequest patronRequest, String action, Object parameters, boolean userGenerated = false) {
        Map result = [ : ];
        ...
        if (patronRequest == null) {
            ...
            if (patronRequest.isNetworkActivityIdle()) {
                if (isValid(patronRequest, action) || (patronRequest.isRequester && Actions.ACTION_REQUESTER_EDIT.equals(action))) {
                    ...
                    ActionResultDetails resultDetails = performAction(action, patronRequest, parameters, userGenerated);
                    ...
                    illApplicationEventHandlerService.auditEntry(patronRequest, patronRequest.state, patronRequest.state, result.message, null);
     ...
     */
    ActionResultDetails performAction(String action, PatronRequest request, Object parameters, boolean userGenerated = false) {
		log.trace("actionService::performAction(action: " + action + ", request id: " + request.id + ", parameters: " + parameters.toString() + ")");
		...
		log.trace("actionService::performAction, current state: " + request.state.code);

		// We will allow the action handler to modify the parameters, so we will need to take a copy
		Object parametersToUse = parameters;
		if ((parameters != null) && userGenerated) {
			// We need to take a copy of the map from the controller, so we can update it
			parametersToUse = (parameters instanceof Map) ? new HashMap<String, Object>(parameters) : new HashMap<String, Object>();

			// Only need to set this, when we are coming from the ui			
			resultDetails.auditData = parametersToUse;
		}
        ...
        if (actionBean == null) {
            ...
            resultDetails = actionBean.performAction(request, parametersToUse, resultDetails);
        }

		// Do we need to send a protocol message
		if (resultDetails.sendProtocolMessage) {
			// Is the action applicable for the protocol
			if (request.currentProtocol.actionEvents.find { ProtocolActionEvent protocolActionEvent ->
				return(protocolActionEvent.actionEvent.code == action);
			} != null) {
        ...
        if (resultDetails.overrideStatus == null) {
            ...
            NewStatusResult newResultStatus = statusService.lookupStatus(
				request,
				action,
				resultDetails.qualifier,
				resultDetails.result == ActionResult.SUCCESS,
		...
		log.trace("actionService::performAction, new state: " + request.state.code);
...
//        illApplicationEventHandlerService.auditEntry(request, currentState, request.state, 'Action: ' + action + ', State change: ' + currentState.code + ' -> '  + request.state.code, null);
    ...
    public List getValidActions(PatronRequest request, boolean includeSystem = false) {
        ...
        if (request.isNetworkActivityIdle()) {
            ...
            actions = stateModelService.getValidActions(request.stateModel, request.state, null, includeSystem, request);
