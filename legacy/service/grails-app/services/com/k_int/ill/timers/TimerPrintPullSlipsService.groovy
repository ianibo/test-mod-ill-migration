...
import com.k_int.ill.BatchService;
import com.k_int.ill.EmailService;
import com.k_int.ill.HostLMSLocation;
import com.k_int.ill.OkapiSettingsService;
...
where pr.state in (select sms.state
                   from StateModel as sm
                       inner join sm.states as sms
                   where sm = pr.stateModel and
                         sms.triggerPullSlipEmail = true) and
...
''';
    ...
    public void performTask(String tenant, Institution institution, String config) {
        ...
        checkPullSlips(tenant, institution, task_config);
    }

    private void checkPullSlips(String tenant, Institution institution, Map timer_cfg) {
     ...
     * @param institution The institution we are checking for pullslips against
    ...
    private void checkPullSlipsFor(
        ...
        try {
            ...
            String pullSlipTemplateId = institutionSettingsService.getSettingValue(
                institution,
                SettingsData.SETTING_PULL_SLIP_TEMPLATE_ID
            );
            ...
            if (pullSlipTemplateId == null) {
                ...
                if (tc != null) {
                    ...
                    Map summaryParameters = [
                        institution : institution
                    ]
                    def pull_slip_overall_summary = PatronRequest.executeQuery(PULL_SLIP_SUMMARY, summaryParameters);
                    ...
                    List<HostLMSLocation> pslocs = HostLMSLocation.executeQuery(LOCATIONS_QUERY, locationParameters);
                    ...
                    if (( pslocs.size() > 0) && (emailAddresses != null) && (emailAddresses.size() > 0)) {
                        ...
                        Map pullSlipQueryParameters = [
                            loccodes : loccodes,
                            institution : institution
                        ];
                        List<PatronRequest> pending_ps_printing = PatronRequest.executeQuery(PULL_SLIP_QUERY, pullSlipQueryParameters);
                        ...
                        if ((pending_ps_printing != null) && (pending_ps_printing.size() > 0)) {
                            ...
                            def tmplResult = templatingService.performTemplate(
                                ...
                                [
                                    ...
                                    illURL: "${okapiSettingsService.getSetting('FOLIO_HOST')?.value}/supply/requests?filters=${locFilters}%2C${stateFilters}&sort=-dateCreated"
                                ],
                                Template.LOCALITY_ENGLISH
                            );
                            ...
                            if (attachPullSlips) {
                                ...
                                int numberOfRequestsPerPullSlip = reportService.getMaxRequestsInPullSlip(institution);
                                ...
                                String reportId = reportService.getPullSlipReportId(institution);
                                int startPosition = 1;
                                requestMapsList.each { Map requestMap ->
                                    ...
                                    Map batchResult = batchService.generatePickListBatchFromList(institution, requestMap.values().toList(), "Created (" + startPosition.toString() + "):", true, true);
