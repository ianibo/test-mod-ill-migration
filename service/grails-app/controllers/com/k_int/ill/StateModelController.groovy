...
import com.k_int.ill.statemodel.StateModelService;
...
@Slf4j
@CurrentTenant
@Api(value = "/ill/stateModel", tags = ["State Model"])
@OkapiApi(name = "stateModel")
@ExcludeFromGeneratedCoverageReport
public class StateModelController extends OkapiTenantAwareController<StateModel>  {

    private static final String RESOURCE_STATE_MODEL = StateModel.getSimpleName();

    StateModelService stateModelService;

	public StateModelController() {
		super(StateModel)
	}
    ...
    @OkapiPermission(name = "getValidActions", permissionGroup = PermissionGroup.READ)
    public def getValidActions() {
    ...
    @OkapiPermission(name = "export", permissionGroup = PermissionGroup.READ)
	public def export() {
        ContextLogging.startTime();
        ContextLogging.setValue(ContextLogging.FIELD_RESOURCE, RESOURCE_STATE_MODEL);
    ...
    @OkapiPermission(name = "ingest", permissionGroup = PermissionGroup.WRITE)
    public def ingest() {
        ContextLogging.startTime();
        ContextLogging.setValue(ContextLogging.FIELD_RESOURCE, RESOURCE_STATE_MODEL);
