...
import com.k_int.ill.logging.NcipLogDetails;
...
@Slf4j
@Api(value = "/ill/testHostLMS", tags = ["Test Host LMS"])
@OkapiApi(name = "testHostLMS")
@ExcludeFromGeneratedCoverageReport
@CurrentTenant
public class TestHostLMSController extends OkapiTenantAwareInstitutionController<PatronRequest> {
    ...
    @OkapiPermission(name = "validate", permissionGroup = PermissionGroup.READ)
	public def validate() {
        ...
        if (params.lms) {
            ...
            if (hostLMSActions == null) {
                ...
                if (params.patronId) {
                    ...
                    result = hostLMSActions.lookupPatron(
                        getInstitution(),
                        settings,
                        params.patronId,
                        ncipLogDetails
    ...
    @OkapiPermission(name = "checkOut", permissionGroup = PermissionGroup.WRITE)
    public def checkOut() {
        ContextLogging.startTime();
        ...
        if (params.lms) {
            ...
            if (hostLMSActions == null) {
                ...
                if (params.requestId && params.itemBarcode && params.borrowerBarcode) {
                    ...
                    // Now we can make the call
                    INcipLogDetails ncipLogDetails = new NcipLogDetails();
                    result = hostLMSActions.checkoutItem(
                        getInstitution(),
                        settings,
                        params.requestId,
                        params.itemBarcode,
                        params.borrowerBarcode,
                        ncipLogDetails
                    );
                    result.logging = ncipLogDetails.toMap();
                } else {
    ...
    @OkapiPermission(name = "checkIn", permissionGroup = PermissionGroup.WRITE)
    public def checkIn() {
        ContextLogging.startTime();
        ...
        if (params.lms) {
            ...
            if (hostLMSActions == null) {
                ...
                if (params.itemBarcode) {
                    ...
                    // Now we can make the call
                    INcipLogDetails ncipLogDetails = new NcipLogDetails();
                    result = hostLMSActions.checkInItem(
                        getInstitution(),
                        settings,
                        params.itemBarcode,
                        ncipLogDetails
    ...
    @OkapiPermission(name = "accept", permissionGroup = PermissionGroup.WRITE)
    public def acceptItem() {
        ContextLogging.startTime();
    ...
    @OkapiPermission(name = "determineBestLocation", permissionGroup = PermissionGroup.READ)
    public def determineBestLocation() {
        ContextLogging.startTime();
        ContextLogging.setValue(ContextLogging.FIELD_ACTION, ContextLogging.ACTION_TEST_NCIP_ACCEPT_ITEM);
        log.debug(ContextLogging.MESSAGE_ENTERING);
