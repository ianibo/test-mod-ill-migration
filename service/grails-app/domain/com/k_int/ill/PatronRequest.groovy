 ...
 * requesting that ill locate and deliver a resource from a remote partner.
 */
...
class PatronRequest implements CustomProperties, MultiTenant<PatronRequest> {
	...
	@CategoryId(Category.PUBLICATION_TYPE)
	RefdataValue publicationType

	@Defaults(['ArticleExchange', 'Courier', 'Email', 'FTP', 'Mail', 'Odyssey', 'URL'])
	@CategoryId(Category.DELIVERY_METHOD)
	RefdataValue deliveryMethod
	...
	@Defaults([ServiceType.LOAN, ServiceType.COPY])
	@CategoryId(Category.SERVICE_TYPE)
	...
	/** The current protocol in play */
	Protocol currentProtocol;

	/** The copyright gubbins for this request */
	PatronRequestCopyright copyright;
	...
	static hasMany = [
		...
		documents : PatronRequestDocument,
	...
	static mapping = {
	    ...
	    sentISO18626RequestResponse column: 'pr_sent_iso18626_request_response'
	                    ...
	                    institution column: 'pr_institution_id'
		           current_protocol column: 'pr_current_protocol_id'
				   		  copyright column: 'pr_copyright', cascade: 'all-delete-orphan'

	                           rota cascade: 'all-delete-orphan', sort:'rotaPosition', order:'asc'
	                          audit cascade: 'all-delete-orphan', sort:'dateCreated', order:'desc'
	                  protocolAudit cascade: 'all-delete-orphan', sort:'dateCreated', order:'desc'
  ...
  def beforeInsert() {
	...
	log.trace("PatronRequest::beforeInsert: Statemodel set to: " + this.stateModel.shortcode + ", ServiceType: " + serviceType?.label + ", currentProtocol: " + currentProtocol?.id);
  ...
  public int incrementLastSequence() {
	...
	List getValidActions() {
		// We are called from the view, but unfortunatly from grails 5 we no longer have a session so we have to generate one
		// We ahould not be called from anywhere other than a view, as all other places should call actionService.getValidActions directly
