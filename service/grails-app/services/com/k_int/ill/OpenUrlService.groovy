...
import com.k_int.ill.constants.IdentifierType;
...
public class OpenUrlService {
    ...
    private static final String OPEN_URL_01_SHARED_INDEX_ID              = 'sharedIndexId';
     ...
     */
    public GenericResult mapToRequest(Map parameters) {
     ...
     * @param result The GenericResult object that is passed back to the caller
     * @param req The req section of the OpenURL 1.0 request
     */
    private void mapReqToRequest(PatronRequest request, GenericResult result, Map req) {
     ...
     */
    private void mapRftToRequest(PatronRequest request, GenericResult result, Map rft) {

        // Cannot do anything if the map is null
        if (rft != null) {
            request.publicationType = refdataValueService.lookupPublicationType(defaultToNullIfEmpty(rft, OPEN_URL_10_RFT_GENRE));
     ...
     */
    private void mapSvcToRequest(PatronRequest request, GenericResult result, Map svc) {
        ...
        if (svc != null) {
            ...
            request.serviceType = refdataValueService.lookupServiceType(defaultToNullIfEmpty(svc, OPEN_URL_10_SVC_SERVICE_TYPE));
        }
    }
     ...
     */
    private void mapOpenURL01ToRequest(PatronRequest request, GenericResult result, Map parameters) {
        // Cannot do anything if the map is null
        if (parameters != null) {
			...
			// The shared index id is specific to us
			request.systemInstanceIdentifier = defaultToNullIfEmpty(parameters, OPEN_URL_01_SHARED_INDEX_ID);
        }
    }
     ...
     * @param result The GenericResult object that is passed back to the caller
     */
    private void checkNumberOfPages(PatronRequest request, String endPage, GenericResult result) {
     ...
     */
    private boolean verifyLocations(PatronRequest request, GenericResult result) {
            ...
            if (pickupEntry == null) {
                ...
                if (institutionEntry == null) {
                    ...
                    if (institutionEntry.symbols) {
                        ...
                        // Set the institution
                        request.institution = institutionService.getInstitution(institutionEntry);
                    } else {
                        result.result = ActionResult.ERROR;
    ...
    private DirectoryEntry findInstitution(DirectoryEntry entry) {
        ...
        while ((potentialInstitution != null) && !institutionFound) {
            ...
            if (potentialInstitution.type && (potentialInstitution.type.value == 'institution')) {
