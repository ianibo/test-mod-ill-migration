...
public class RequestRouterService {
	...
	public RequestRouter getRequestRouterFor(String impl) {
		...
		if ( result == null ) {
			...
			log.debug("Got ${result.getRouterInfo().toString()}");
		}

		return result;
	}

	public RequestRouter getRequestRouter(Institution institution) {
		RequestRouter result = null;
		String v = institutionSettingsService.getSettingValue(
			institution,
			SettingsData.SETTING_ROUTING_ADAPTER
		);
	...
	public List<RankedSupplier> findMoreSuppliers(
		...
		// around line 222 of IllApplicationEventHandlerService.groovy. Our aim is to hide the
		// details of finding a list of potential suppliers so different implementations can be used
		RequestRouter rr = getRequestRouter(institution);
		if ( rr != null ) {
			supplier_list = rr.findMoreSuppliers(patronRequest);
		} else {
