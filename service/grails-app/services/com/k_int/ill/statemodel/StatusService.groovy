...
public class StatusService {
    ...
    public List<Transition> possibleActionTransitionsForModel(StateModel model, ActionEvent action, boolean traverseHierarchy = true) {
        ...
        if ((action != null) && (model != null)) {
			// build up the list of valid states
			List<Status> validToStates = new ArrayList<Status>();
			model.states.each { StateModelStatus stateModelStatus ->
				validToStates.add(stateModelStatus.state);
			}
     ...
     * @param validToStatus The valid list of status that the to status can be
     * @param model The model that we want the tansitions for
     * @param action The action that we want the transitions for
     * @param traverseHierarchy Do we traverse the inheritance hierarchy or not
     */
    private void transitionsForModel(
		List<Transition> transitions,
		List<Status> validToStatus,
		StateModel model,
		ActionEvent action,
        ...
        if (traverseHierarchy && model.inheritedStateModels) {
            ...
            model.inheritedStateModels.each { inheritedStateModel ->
                ...
                transitionsForModel(
					transitions,
					validToStatus,
					inheritedStateModel.inheritedStateModel,
					action,
					traverseHierarchy
				);
            }
        ...
        Status toStatus)
    {
        ...
        if (fromStatus && actionEvent && toStatus) {
			// Check to see if it is a valid to state first
			if (validToStatus.contains(toStatus)) {
    ...
    private ActionEventResult findResult(StateModel model, Status fromStatus, String actionCode, boolean successful, String qualifier, boolean availableActionMustExist) {
		log.trace("findResult(stateModel: " + model?.shortcode + ", fromStatus: " + fromStatus?.code + ", actionCode: " + actionCode + ", successful: " + successful.toString() + ", qualifier: " + qualifier + ", successful: " + successful.toString() + ", availableActionMustExist: " + availableActionMustExist.toString());
                ...
                if (availableActionMustExist) {
                    ...
                    if (availableAction != null) {
                        ...
                        if (availableAction.resultList != null) {
							...
							log.trace("findResult - result found from availableAction.resultList.lookupResult: " + actionEventResult?.code);
                        }
                    } else {
                        carryOn = false;
                ...
                if (carryOn) {
                    ...
                    if (actionEventResult == null) {
                        ...
                        if (actionEvent.resultList != null) {
							...
							log.trace("findResult - result found from actionEvent.resultList.lookupResult: " + actionEventResult?.code);
                        }
		...
		log.trace("return from findResult: " + actionEventResult?.code);
    ...
    public NewStatusResult lookupStatus(PatronRequest request, String action, String qualifier, boolean successful, boolean availableActionMustExist) {
		log.trace("lookupStatus(hrid: " + request.hrid + ", action: " + action + ", qualifier: " + qualifier + ", successful: " + successful.toString() + ", availableActionMustExist: " + availableActionMustExist.toString());
        ...
        ActionEventResult actionEventResult = findResult(request.stateModel, request.state, action, successful, qualifier, availableActionMustExist);
		...
		log.trace("return from lookupStatus: " + newStatusResult.toString());
    ...
    public StateModel getStateModel(PatronRequest request) {
		...
		// Is this a requester side request
        if (request.isRequester) {
			// Is it one that is deemed a copy
			if (patronRequestService.isCopy(request)) {
				// It is, so used the requester copy state model
                settingsKey = SettingsData.SETTING_STATE_MODEL_REQUESTER_COPY;
			} else if (request?.deliveryMethod?.value == 'url') {
				// It is one that is deemed to be a CDL type request
                settingsKey = SettingsData.SETTING_STATE_MODEL_REQUESTER_CDL;
            } else {
				// Finally we fall back to loan
                settingsKey = SettingsData.SETTING_STATE_MODEL_REQUESTER;
            }
        } else {
			// Is it one that is deemed a copy
			if (patronRequestService.isCopy(request)) {
				// It is, so use the responder state table
                settingsKey = SettingsData.SETTING_STATE_MODEL_RESPONDER_COPY;
			} else if (request?.deliveryMethod?.value == 'url') {
				// It is one that is deemed to be a CDL type request
                settingsKey = SettingsData.SETTING_STATE_MODEL_RESPONDER_CDL;
            } else {
				// Finally we fall back to loan
		...
		// Lookup the state model from the settings
        stateModelCode = institutionSettingsService.getSettingValue(request.institution, settingsKey);
		log.trace("getStateModel: Looked up setting \"" + settingsKey + "\" for institution " + request.institution.id + " and found: " + stateModelCode);
		
        // Lookup the state model
        StateModel stateModel = StateModel.lookup(stateModelCode);
		
		// Did we find the state model
        if (stateModel == null) {
	        if (request.isRequester) {
				// Is it one that is deemed a copy
				if (patronRequestService.isCopy(request)) {
					// It is, so used the requester copy state model
	                stateModelCode = StateModel.MODEL_REQUESTER_COPY;
				} else if (request?.deliveryMethod?.value == 'url') {
					// It is one that is deemed to be a CDL type request
	                stateModelCode = StateModel.MODEL_DIGITAL_RETURNABLE_REQUESTER;
	            } else {
					// Finally we fall back to loan
	                stateModelCode = StateModel.MODEL_REQUESTER;
	            }
	        } else {
				// Is it one that is deemed a copy
				if (patronRequestService.isCopy(request)) {
					// It is, so use the responder state table
	                stateModelCode = StateModel.MODEL_RESPONDER_COPY;
				} else if (request?.deliveryMethod?.value == 'url') {
					// It is one that is deemed to be a CDL type request
	                stateModelCode = StateModel.MODEL_CDL_RESPONDER;
	            } else {
					// Finally we fall back to loan
	                stateModelCode = StateModel.MODEL_RESPONDER;
	            }
	        }
			...
			stateModel = StateModel.lookup(stateModelCode);
			log.trace("getStateModel: Defaulting statemodel to: " + stateModel?.shortcode);
        }

        // Return the found state model
		log.trace("getStateModel: returning statemodel " + stateModel?.shortcode);
        return(stateModel);
    }
}
