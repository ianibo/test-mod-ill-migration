...
class DirectoryEntrySpec extends TestBase {
    ...
    void "Create a new DirectoryEntry"(
        ...
        when:"Create a new DirectoryEntry"
            ...
            int statusCode = 201;
            try {
                response = doPost("${baseUrl}/ill/directory/entry".toString(), null, null, {
    ...
    void "Fetch a specific DirectoryEntry"(String tenantId, String ignore) {
        when:"Fetch the DirectoryEntry"
            ...
            def response = doGet("${baseUrl}/ill/directory/entry/" + testctx.directoryEntry.id.toString());
    ...
    void "Search for DirectoryEntryies"(String tenantId, String ignore) {
        when:"Search for DirectoryEntries"
            ...
            // Perform a search
            def response = doGet("${baseUrl}/ill/directory/entry", [ filters : "slug==" + testctx.directoryEntry.slug ]);
    ...
    void "Update DirectoryEntry name"(String tenantId, String name) {
        when:"Update name for DirectoryEntry"
            ...
            setHeaders(
				[
					 'X-Okapi-Tenant': tenantId,
					 'X-Okapi-Permissions': '[ "ill.directory.entry.item.put", "ill.directory.entry.managed-item.put", "ill.directory.entry.item-local.put" ]'
				]
			);

            // Perform an update
			log.error("Chas: Performing a put to ${baseUrl}ill/directory/entry/" + testctx.directoryEntry.id.toString() + ", with \n" + json);
            def response = doPut("${baseUrl}ill/directory/entry/" + testctx.directoryEntry.id.toString(), null, null, {
                // Note: request is of type groovyx.net.http.HttpConfigs$BasicRequest
                request.setBody(json);
            });
    ...
    void "Delete a DirectoryEntry"(String tenantId, String ignore) {
        when:"Delete a DirectoryEntry"
            ...
            try {
                response = doDelete("${baseUrl}/ill/directory/entry/" + testctx.directoryEntry.id.toString());
            } catch (groovyx.net.http.HttpException e) {
                statusCode = e.getStatusCode();
                response = e.getBody();
            }
