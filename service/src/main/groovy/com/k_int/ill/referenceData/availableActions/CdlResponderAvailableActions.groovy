...
/**
 * Creates the available actions for the CDL Responder state model
 * @author Chas
 *
 */
@Slf4j
public class CdlResponderAvailableActions extends BaseAvailableActions {

    public void load(StateModel stateModel) {
        log.info('Adding available actions for the CDL Responder state model');

        // RES_AWAIT_PICKING OR "Searching"
        AvailableAction.ensure(stateModel, Status.RESPONDER_AWAIT_PICKING, Actions.ACTION_RESPONDER_SUPPLIER_CHECK_INTO_ILL, AvailableAction.TRIGGER_TYPE_MANUAL, ActionEventResultList.CDL_RESPONDER_CHECK_INTO_ILL)

        // RES_SEQUESTERED
        AvailableAction.ensure(stateModel, Status.RESPONDER_SEQUESTERED, Actions.ACTION_RESPONDER_SUPPLIER_FILL_DIGITAL_LOAN, AvailableAction.TRIGGER_TYPE_MANUAL)

        // RES_LOANED_DIGITALLY
        AvailableAction.ensure(stateModel, Status.RESPONDER_AWAIT_DESEQUESTRATION, Actions.ACTION_RESPONDER_SUPPLIER_CHECKOUT_OF_ILL, AvailableAction.TRIGGER_TYPE_MANUAL)
